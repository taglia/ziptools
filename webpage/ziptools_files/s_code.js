 ...
 * 2012-01-02
   BI: aggiunto 'pmk' in getQueryParam() prop19

 * 2012-01-05
   BI: retrocompatibilita' 'HP' + 'pmk' in getQueryParam() prop19

 * 2012-01-23
   BI: tracciamento cookie np_id

 * 2012-02-01
   BI: aggiunta s_trackPosition per tracciamento click

 * 2012-02-07
   BI: prop40 per fullPageName, ad eccezione di "Virgilio Local" dove è la prop57
       eVar57 per url

 * 2012-02-15
   BI: modifiche per funnel Mail Virgilio
       eVar60, con valore del precedente pagename per funnel Mail Virgilio
       eVar61, con valore del parametro "entryPoint" per funnel Mail Virgilio

 * 2012-03-27
   BI: forzo _full = true per report suite Virgilio Local, per eliminare definitivamente gli Unique Exceeded
 
 * 2012-04-11
   BI: rimozione cookie np (bug_id 9212)
 
 * 2012-06-19
   BI: nuova gerarchia h3

 * 2012-09-13
   BI: tracciamento traffico under login
...
var s_account='tivirgilioglobalpreprod';
var s_linkInternalFilters='javascript:,virgilio.it';

if (typeof(mtx_account)!="undefined"){s_account=mtx_account;}
if (typeof(mtx_linkInternalFilters)!="undefined"){s_linkInternalFilters=mtx_linkInternalFilters;}
...
function mtx_fn_clone_prop(s,mtx){
	var i,k;
	for (i=1;i<75;i++){
		k="prop"+i;
		if (mtx[k]){s[k]=mtx[k];}
...
/* gcasati 20120201 probabilmente la eVar49 e eVar50 sono dei refusi */
s.linkTrackVars="prop58,prop59,eVar49,eVar50,eVar58,eVar59";
...
function s_doPlugins(s){
	...
	if(typeof(_full)=="undefined"){_full=true;}
	if(mtx_s.channel){s.channel=mtx_s.channel;}
	if(mtx_s.pageName){s.pageName=mtx_s.pageName;}else{s.pageName="VIR:"+_hostname;}
	...
	if(mtx_s.savePageName){
		...
		if(s_account=="tivirgilioglobalprod,tivirlocalprod"){
			...
			s.prop40=s.pageName+":"+mtx_fn_path(_path);
		}
	}
	...
	if(s_oMetaData&&s_oMetaData.length>0){
		for(var i=0;i<s_oMetaData.length;i++){
			if(s_oMetaData[i].content.indexOf('charset=')>-1){
				var s_sCSet=s_oMetaData[i].content.split('=');
				s.charSet=s_sCSet[1];
			}
		}
	}
	...
	if(s_account=="tivirgilioglobalprod,tivirmailservprod"){
	...
	if((s.prop9)&&(window.LOCAL_COMMON)){
		if(s.prop9.toLowerCase()=="local"){
			if (LOCAL_COMMON.idCity){
				if(!s.prop38)s.prop38=LOCAL_COMMON.idCity;
				if(s.prop38)s.eVar38='D=c38';
			}
			if(LOCAL_COMMON.idProv){
				if(!s.prop40)s.prop40=LOCAL_COMMON.idProv;
				if(s.prop40)s.eVar40='D=c40';
			}
			if(LOCAL_COMMON.idReg){
				if(!s.prop55)s.prop55=LOCAL_COMMON.idReg;
				if(s.prop55)s.eVar55='D=c55';
			}
			if(LOCAL_COMMON.istatCommerciale){
				if (!s.prop43)s.prop43=LOCAL_COMMON.istatCommerciale;
	...
	/* Impostazione idBanner Timinternet */
	if(!s.prop42){
		s.prop42=s.getQueryParam('idBanner');
		if(!s.prop42){s.prop42='None';}
		s.eVar42='D=c42';
	}

        /* Tracciamento traffico under login */
        if(!s.prop69){
                s.prop69=getOmnitureCookie('tinv')||'ND';
		s.eVar69='D=c69';
        }
	...
	/* Campagne interne */
	if(!s.eVar15){s.eVar15=s.getQueryParam('icid');}

	/* Aggregati */
	var s_aSez1;
	var s_ioPub="iopubblicita.it/";
	var s_1254="http://1254.virgilio.it/";
	var s_dlh=document.location.href;
	//Aggregato 1
	if(s.prop9&&!s.eVar9){s.eVar9=s.prop9;}
	//Aggregato 2
	if(s.channel){
		if(s.channel.toLowerCase()=="contenuti local"){
			if(!s.prop13){
				s_aSez1=["annunci","blog","fotoalbum"];
				s.prop13=s_setAggregato(s_aSez1,"Community");
			}
			if(!s.prop13){
				s_aSez1=["cinema","eventi","notizie","infotraffico","pubblica utilita"];
				s.prop13=s_setAggregato(s_aSez1,"Editoriale");
			}
		}
		//Aggregato 3
		// 22-06-2011 Modifica logica della prop14 Aggregato 3 per includere tutto quello che ï¿½ in local (prop9) ad esclusione della HP 1254
		if(!s.prop14){
			if(s.prop9.toLowerCase()=="local" && s_dlh != s_1254){s.prop14="Local Senza 1254";}
		}
		//Aggregato 4
		if(s.channel.toLowerCase()=="directories"){
			if(!s.prop15){
				if(!s.prop1){s.prop15="Macrodirectories";}
				else{
					s_aSez1=["categorie aziende","index","listato aziende","schede","promozioni"];
					s.prop15=s_setAggregato(s_aSez1,"Macrodirectories");
				}
			}
		}
	}
	//Aggregato 2...continua
	if(!s.prop13){
		s_aSez1=["virgilio football people"];
		s.prop13=s_setAggregato(s_aSez1,"Football");
	}
	if(!s.prop13){
		if(s.prop2&&s.prop2.toLowerCase()=="profili football"){s.prop13="Football";}
	}

	//Aggregato 4...continua
	if(!s.prop15){
		if(s_dlh==s_1254){s.prop15="Macrodirectories";}
		if(s_dlh.indexOf(s_ioPub)>-1){
			if(s_dlh!="http://www."+s_ioPub&&s_dlh!="http://"+s_ioPub){
				s.prop15="Macrodirectories";
			}
		}
	}
...
/* Internal Search */
	...
	// Hierarchies
	if(!s.hier1){
		if(s.channel){s.hier1=s.apl(s.hier1,s.channel,',',0);}
		for(var i=1;i<9;i++){
			if(s['prop'+i]){s.hier1=s.apl(s.hier1,s['prop'+i],',',0);}
		}
	}
	...
	if(!s.hier3&&s.prop16){
		...
		for(var i=1;i<9;i++){
			if(s['prop'+i]){s.hier3=s.apl(s.hier3,s['prop'+i],',',0);}
		}
	}
...
function s_setAggregato(aSez,sAggr){
	if(arguments.length!=2) return "";
	if(s.prop1) {
		for(var i in aSez) {
			if(s.prop1.toLowerCase()==aSez[i]) return sAggr;
		}
	}
	return "";
}
